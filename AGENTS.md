...
- Seitenstruktur: Kurzerklärung → Preistabelle (Sortierung je Spalte, horizontales Scrollen per Drag-to-Scroll `setupDragScroll` auf dem `overflow-x-auto`-Container, Fähigkeiten-Badges-Spalte, Fähigkeiten-Filter-Toggles) → **Model-Ranking** (`ModelRanking`: Top-10-Modelle nach Anfragen/Monat beim vollen Guthaben, gleiche Berechnung wie die Tabelle) → Free-Models-Tabelle (neuestes oben, eigene unabhängige Fähigkeiten-Filter-Toggles) → Datenschutz-Tabelle (`PrivacyTable`: Sortierung Modell/Stufe, Default `tier:asc` = schlechteste Stufe oben; Badges `badge-error` Modelltraining / `badge-warning` Aufbewahrung > 0 Tage und „Kein ZDR“ (Daten aufbewahrt, Dauer unbekannt) / `badge-success` ZDR / `badge-ghost` keine Angabe; „≈“ = Familien-Fallback; „Gültig bis“-Spalte = `validUntil` oder „bis auf weiteres“) → **FAQ** (`Faq`: native `<details>`, Texte aus `src/data/faq.json`, `{credit}`/`{cost}` via `fmtPricing`) → Changelog (JSON-Events, i18n-Texte, Badges) → Impressum/Datenschutz.
...
- **Head-SEO (build-generiert in `prerender.mjs`):** Title/Description/Canonical/`og:locale` je Sprache, `hreflang`, RSS-Autodiscovery (`releases.atom`), JSON-LD (`WebSite` + `ItemList` aller Modelle + `FAQPage`). Zusätzlich `dist/robots.txt` und `dist/sitemap.xml` (beide Sprachen).
...
- **Quellen der Inhalte:** Modell-Ranking (`src/components/ModelRanking.tsx`, `requestsPerMonth`/`requestCost`) und FAQ (`src/components/Faq.tsx` + `src/data/faq.json`, zweisprachig) sind server- und clientseitig identisch (keine `Date`/`window`-Abhängigkeit im Rendering).
