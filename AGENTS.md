...
- `freeModels` = kostenlose Zen-Modelle aus der Zen-Doku `https://opencode.ai/docs/de/zen/` („Endpunkte"-Tabelle liefert die Model-IDs, „Preise"-Tabelle markiert die gratis Zeilen via `extractFreeModelsFromDocs`); `availableFrom` = erstes Beobachtungsdatum (bleibt über Läufe erhalten).
